import type { ThematicPageData } from "../../types/thematic";

export const qatarBanking: ThematicPageData = {
  slug: "banking",
  theme: "banking",
  countryKey: "qatar",
  countrySlug: "qatar",
  countryName: "Qatar",
  title: "Banking in Qatar: Accounts, Banks & Financial Guide for Expats",
  description:
    "Guide to banking in Qatar for expats. Compare QNB, Commercial Bank, Doha Bank, QIIB, and Masraf Al Rayan. Learn about account types, WPS salary transfers, credit cards, and QCB regulations.",
  intro: `Qatar's banking sector is well-developed, tightly regulated, and dominated by a mix of conventional and Islamic financial institutions. The Qatar Central Bank (QCB) oversees all banking operations in the country and enforces strict capital adequacy, anti-money laundering, and consumer protection standards. For expats, opening a bank account in Qatar is straightforward and usually a requirement of the employment process, as salaries must be paid through the Wage Protection System (WPS).

The WPS is a government-mandated electronic salary transfer system that ensures employers pay workers on time and in full. All employers in Qatar are required to transfer salaries through WPS via a registered bank. This means expats need a Qatari bank account, typically opened during the first few weeks of employment. Most banks offer dedicated expat account packages with reduced minimum balance requirements and bundled services.

Qatar is home to both conventional and Islamic (Sharia-compliant) banks. Islamic banks do not charge or pay interest; instead, they use profit-sharing, leasing, and cost-plus financing structures that comply with Islamic principles. The largest Islamic bank in Qatar is Qatar Islamic Bank (QIB), followed by Masraf Al Rayan and Qatar International Islamic Bank (QIIB). Conventional banks include Qatar National Bank (QNB), the largest bank in the Middle East and Africa by assets, Commercial Bank of Qatar, and Doha Bank. Expats can choose either type based on personal preference and financial needs.`,
  citySections: [
    {
      city: "Doha",
      slug: "doha",
      content: `Doha is the financial capital of Qatar, and all major banks have their headquarters and primary branch networks in the city. Qatar National Bank (QNB) is the dominant player, with over 60 branches and ATMs across Doha. QNB offers expat-friendly current accounts, savings accounts, credit cards, personal loans, and car financing. Opening a QNB account requires a valid QID, passport, employment contract or salary certificate, and an NOC from your employer. The minimum salary requirement for a standard current account is typically QAR 3,000-5,000 (~€750-1,250) per month.

Commercial Bank of Qatar (CBQ) is the second-largest conventional bank and is popular among expats for its digital banking platform and customer service. CBQ offers a range of credit cards with air miles, cashback, and travel insurance benefits. Its Combi Savings Account is a popular choice for expats wanting to save in multiple currencies. Branches are concentrated in West Bay, Al Sadd, and The Pearl.

Qatar International Islamic Bank (QIIB) is a strong choice for expats interested in Sharia-compliant banking. QIIB offers current and savings accounts, home financing, vehicle financing, and credit cards structured around Islamic finance principles. The bank has a modern mobile app and branches throughout Doha, with particularly strong presence in the Al Sadd and Bin Mahmoud areas.

Doha Bank, despite its name, operates across Qatar but has a significant presence in central Doha. It offers competitive foreign exchange rates and international transfer services, making it popular with expats who regularly send money abroad. Doha Bank's remittance service is competitively priced and supports transfers to most countries in Asia, Africa, and Europe.

Masraf Al Rayan is the second-largest Islamic bank in Qatar and has grown rapidly in recent years. It offers personal finance products, credit cards, and investment accounts that comply with Sharia principles. The bank is known for its Visa Infinite card, which comes with premium travel and lifestyle benefits. Masraf Al Rayan operates branches in major Doha areas including Lusail, West Bay, and Al Gharrafa.

For international money transfers, expats in Doha can use traditional bank wire transfers, dedicated remittance services, or digital platforms. Exchange houses like Al Dar, Al Jazeera, and Gulf Exchange operate in most malls and commercial areas. Digital services like Wise (formerly TransferWise) and Western Union's online platform are also used, though fees and exchange rate margins vary. Bank wire transfers from QNB or CBQ typically cost QAR 40-75 (~€10-19) per transaction for international transfers. Most expats also use their bank's mobile app for local transfers, bill payments, and account management.`,
    },
  ],
  faqs: [
    {
      question: "What documents do I need to open a bank account in Qatar?",
      answer:
        "To open a bank account in Qatar, you typically need a valid QID (Qatar ID card), your passport with a valid residence visa, a salary certificate or employment contract from your employer, and an NOC (No Objection Certificate) from your sponsor or employer. Some banks also require proof of address such as a Baladiya-registered tenancy contract. The process usually takes 1-3 business days, and most banks offer same-day debit card issuance.",
    },
    {
      question: "What is the Wage Protection System (WPS) in Qatar?",
      answer:
        "The Wage Protection System (WPS) is an electronic salary transfer mechanism mandated by Qatar's Ministry of Labour. It requires all employers to pay employee salaries through authorised banks and financial institutions, ensuring timely and accurate payment. WPS allows the government to monitor wage payments and identify employers who delay or underpay salaries. As an expat, your salary must be deposited into a Qatari bank account registered under the WPS system.",
    },
    {
      question:
        "What is the difference between conventional and Islamic banks in Qatar?",
      answer:
        "Conventional banks operate on an interest-based model, paying interest on deposits and charging interest on loans. Islamic banks comply with Sharia law, which prohibits interest (riba). Instead, they use profit-sharing (Mudaraba), cost-plus financing (Murabaha), and leasing (Ijara) structures. For everyday banking such as current accounts, debit cards, and transfers, the practical experience is very similar. The differences become more apparent with savings products and financing, where Islamic banks offer profit rates rather than interest rates.",
    },
    {
      question: "Can I get a credit card as an expat in Qatar?",
      answer:
        "Yes, most Qatari banks offer credit cards to expats with a valid QID and a minimum monthly salary (usually QAR 5,000-10,000 / ~€1,250-2,500 for basic cards, QAR 15,000-20,000 / ~€3,750-5,000 for premium cards). Credit limits are typically set at 2-3 times your monthly salary. Popular cards offer air miles (QNB, CBQ), cashback (Doha Bank), or lifestyle rewards (Masraf Al Rayan). Annual fees range from QAR 0 (~€0) for basic cards to QAR 1,500 (~€375) for premium metal cards. Always check the interest rate on outstanding balances, which can range from 2-3.5% per month.",
    },
    {
      question: "How can I send money abroad from Qatar?",
      answer:
        "Expats can send money abroad through bank wire transfers (QAR 40-75 / ~€10-19 per transaction), exchange houses (Al Dar, Gulf Exchange, Al Jazeera Exchange), or digital platforms like Wise and Western Union. Bank transfers offer the best rates for large amounts but are slower (2-5 business days). Exchange houses offer same-day or next-day transfers with competitive rates for popular corridors like India, Philippines, Egypt, and Pakistan. Always compare the total cost including both the fee and the exchange rate margin.",
    },
  ],
  sources: [
    {
      name: "Qatar Central Bank (QCB)",
      url: "https://www.qcb.gov.qa/",
    },
    {
      name: "Qatar National Bank (QNB)",
      url: "https://www.qnb.com/",
    },
  ],
  relatedLinks: [
    { label: "Qatar Expat Guide", href: "/qatar/guide/" },
    { label: "Cost of Living in Qatar", href: "/qatar/cost-of-living/" },
    { label: "Living in Doha", href: "/qatar/doha/" },
    { label: "Visa & Residency in Qatar", href: "/qatar/visa/" },
    { label: "Housing in Qatar", href: "/qatar/housing/" },
    { label: "Banking in the UAE", href: "/uae/banking/" },
    { label: "Banking in Saudi Arabia", href: "/saudi-arabia/banking/" },
  ],
  calculator: "banking",
};
